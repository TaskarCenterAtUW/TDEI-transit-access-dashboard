...
def build_overpass_query(bounds):
    ...
    # Keep in sync with filter_amenities additional_tags below (shop=supermarket, healthcare=*, leisure=park)
    additional_tags = {
        "shop": ["supermarket"],
        "healthcare": ["*"],
        "leisure": ["park"],
...
def _pick_grid(bounds):
    """
    Pick (n_cols, n_rows) for the given bounds.

    Priority:
    1) Explicit OVERPASS_GRID_COLS / OVERPASS_GRID_ROWS
    2) Target tile area (km^2) via OVERPASS_TARGET_TILE_AREA_KM2 (default ~12)
       -> keeps tile area roughly constant across cities (Seattle vs Spokane, etc.)
    """
    cols_env = os.getenv("OVERPASS_GRID_COLS")
    rows_env = os.getenv("OVERPASS_GRID_ROWS")
    if cols_env and rows_env:
        return max(1, int(cols_env)), max(1, int(rows_env))

    target_km2 = float(os.getenv("OVERPASS_TARGET_TILE_AREA_KM2", "12"))
    bbox_km2 = _bbox_area_km2(bounds)
    # Compute an approximate tile count, then choose cols/rows to match bbox aspect ratio.
    n_tiles = max(1, int(math.ceil(bbox_km2 / max(0.1, target_km2))))

    minx, miny, maxx, maxy = bounds
    # aspect ratio in degrees is good enough for layout (we only need a sensible split)
    w = max(1e-9, maxx - minx)
    h = max(1e-9, maxy - miny)
    aspect = w / h

    n_cols = max(1, int(math.ceil(math.sqrt(n_tiles * aspect))))
    n_rows = max(1, int(math.ceil(n_tiles / n_cols)))
...
def query_overpass_tiled_merged(bounds, cache_dir=None):
    ...
    """
    minx, miny, maxx, maxy = bounds
    n_cols, n_rows = _pick_grid(bounds)
    ...
    try:
        bbox_km2 = _bbox_area_km2(bounds)
        print(f"Overpass bbox area ~ {bbox_km2:.1f} km²; target tile area ~ {float(os.getenv('OVERPASS_TARGET_TILE_AREA_KM2', '12')):.1f} km²")
    except Exception:
        pass
    print(
...
def process_geojson_files_in_folder(folder_path):
    ...
    # OSM: supermarket → shop=supermarket; generic healthcare → healthcare=*; parks → leisure=park
    additional_tags = {
        "shop": ["supermarket"],
        "healthcare": ["*"],
        "leisure": ["park"],
